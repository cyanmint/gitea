...
func Register(ctx *context.APIContext) {
	...
	if !password.IsComplexEnough(form.Password) {
		ctx.APIError(http.StatusBadRequest, fmt.Errorf("password does not meet complexity requirements (must include required character types as configured by the administrator)"))
		return
	}
	if err := password.IsPwned(ctx, form.Password); err != nil {
		...
		ctx.APIError(http.StatusBadRequest, fmt.Errorf("password has been found in a data breach; choose a different password"))
		return
	}
	if !user_model.IsEmailDomainAllowed(form.Email) {
		ctx.APIError(http.StatusUnprocessableEntity, fmt.Errorf("email domain is not allowed on this instance"))
		return
	}
